...
#define _FVIM   3  //  Personal              VIM layer
#define _DVIM   4  //  Delete                VIM layer
/*
...
*/
#define _CVIM   5  //  power edition         VIM layer
#define _XVIM   6  //  movement              VIM layer
#define _ZVIM   7  //  alignment             VIM layer
#define _MOUS   8  //  mouse                     layer
#define _ACCN   9  //  gherkin ACCENTS           layer 
#define _SYMB  10  //  gherkin symbols           layer 
#define _FUNC  11  //  gherkin functions         layer 
#define _DALY  12  //  gherkin DAiLY commands    layer 
#define _POWR  13  //  POWER        productivity layer
#define _APPS  14  //  APPlicationS              layer
...
#define COLOR_ZVIM = HSV_PINK      // ( 320,  255,  255)  // 13
...
// TAP DANCE KEYCODES ACCESSIBLE FROM _NUMB (NUMBERS LAYER) 3
    ,SFT_SP  // spacebar when tapped or LSHIFT when holded on numbers layer
...
// MACROS FOR DEFAULT LAYER 0
    ,MY_RESET
    ,THUMB_L2_FVIM_FVIM_MOUS_DVIM_FUNC
    ,THUMB_L4_POWR_DALY_RESET

    ,THUMB_R2_SP_FVIM_MOUS_DVIM_FUNC
    ,THUMB_R4_DALY_FVIM_RGBL
    ,THUMB_R5_ACCN
...
// _ZVIM LAYER
                    ,ZVIM_I  ,ZVIM_O
            ,ZVIM_J ,ZVIM_K  ,ZVIM_L  ,ZVIM_SP
            ,ZVIM_M ,ZVIM_ES ,ZVIM_BS ,ZVIM_EN
/*
*/
// _zvim layer
//////////////////////////////////////////////////////////////////////////////////////////////////////###
...
// [FUNCTIONS] [_DFLT] KC_R (THUMB_L4_POWR_DALY_RESET)process_record_user               //
...
//                                                                                      //
// [functions] [_powr] kc_r   (my_reset)process_record_user                             //
// [functions] [_dflt] l_th_4 (thumb_l4_powr_daly_reset)process_record_user             //
//                                                                                      //
// reset my keyboard function                                                           //
//////////////////////////////////////////////////////////////////////////////////////////
...
// [functions] [_dflt] l_th_4 (thumb_l4_powr_daly_reset)                                //
//                                                                                      //
// reset my keyboard function                                                           //
//////////////////////////////////////////////////////////////////////////////////////////
//
...
//////////////////////////////////////////////////////////////////////////////////////////
//                                                                                      //
// [TAPDANCE] [_DFLT] R_THUMB_1 (FN_PVI)                                                //
//                                                                                      //
...
void FN_PVI_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (FN_PVI_tap_state.state) {
    ...
    case DOUBLE_HOLD:   layer_on(_FVIM); break;
  }
}
...
//////////////////////////////////////////////////////////////////////////////////////////
//                                                                                      //
// [TAPDANCE] [_NUMB] KC_S (SFT_SP)                                                     //
//                                                                                      //
//  L S H I F T     /     S P A C E     B A R                                           //
//                                                                                      //
//  KC_N:  * SPACEBAR,                                                                  //
//         @ LEFT SHIFT                                                                 //
//                                                                                      //
//////////////////////////////////////////////////////////////////////////////////////////
//
//instantalize an instance of 'tap' for the 'SFT_SP' tap dance.
static tap SFT_SP_tap_state = {
  .is_press_action = true,
  .state = 0
};

void SFT_SP_finished (qk_tap_dance_state_t *state, void *user_data) {
  SFT_SP_tap_state.state = cur_dance(state);
  switch (SFT_SP_tap_state.state) {
...
void SFT_SP_reset (qk_tap_dance_state_t *state, void *user_data) {
  switch (SFT_SP_tap_state.state) {
  ...
  SFT_SP_tap_state.state = 0;
}
//                                                                                      //
// [tapdance] [_numb] kc_s (sft_sp)                                                     //
//                                                                                      //
//  c o n t r o l     /     d e l e t e                                                 //
//////////////////////////////////////////////////////////////////////////////////////////
//
//                                                                                      //
//             t a p   d a n c e   f o r    [ _ n u m b ]  l a y e r                    //
...
// TIME 70 is a right time for typing very fast !!!
 ,[SFT_SP] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, SFT_SP_finished, SFT_SP_reset)
// [understanding]
// [_numb] layer
...
//                        ,----------------------------------+---------------------------,   ,------------------------+--------------------,
                                            LT(_APPS, KC_TAB), THUMB_L4_POWR_DALY_RESET,       THUMB_R4_DALY_FVIM_RGBL, THUMB_R5_ACCN,
//                        |----------------------------------+---------------------------|   |------------------------+--------------------|
                                                                                XXXXXXX,                       XXXXXXX,
//                                                           |---------------------------|   |------------------------|
                 MO(_NUMB), THUMB_L2_FVIM_FVIM_MOUS_DVIM_FUNC,                MO(_SYMB),             LT(_SYMB, KC_ENT), THUMB_R2_SP_FVIM_MOUS_DVIM_FUNC, LT(_NUMB, KC_ESC)         
//             '----------+----------------------------------+---------------------------.   .------------------------+--------------------------------+-------------------'
///////////////////////////////////////////////////////////////// ### block ### of lines too long !!! ###
// ###
/*
...
//|--------------+-----------+---------------+--------------+-------------|  |-----------+------+------+------+----------|
    SFT_T(KC_SPC), TD(CTRDEL), ALT_T(KC_COMM), GUI_T(KC_DOT), TD(G_DOEU),         KC_MINS,  KC_4,  KC_5,  KC_6, KC_PLUS,
//|--------------+-----------+---------------+--------------+-------------|  |-----------+------+------+------+----------|
...
//,-----------------------------------------------------------------------.  ,-------------------------------------------.
             KC_1,       KC_2,           KC_3,          KC_4,   KC_5,            KC_6,  KC_7,  KC_8,  KC_9,    KC_0,
//|--------------+-----------+---------------+--------------+-------------|  |-----------+------+------+------+----------|
       TD(SFT_SP),    KC_LCTL,        KC_LALT,       KC_LGUI, KC_DLR,         KC_MINS,  KC_4,  KC_5,  KC_6, KC_PLUS,
//|--------------+-----------+---------------+--------------+-------------|  |-----------+------+------+------+----------|
           KC_ENT,    KC_BSPC,         KC_TAB,       KC_COMM, KC_DOT,      TD(SLNUMB),  KC_1,  KC_2,  KC_3, KC_ASTR,
//,-----------------------------------------------------------------------.  ,-------------------------------------------.
//                                                   ,--------+-----------,  ,--------+----------,
...
//                                                   |--------+-----------|  |--------+----------|
                                                                 _______,      _______,
//                                                            |-----------|  |----------|
                                              _______, _______, SYM_EURO,      _______, _______, _______
//                                          '--------+--------+-----------.  .--------+--------+---------'
),
//'--------------------------------------------------------------------------'  '-------------------------------------------'
///////////////////////////////////////////////////////////////// ### block ### of lines too long !!! ###
...
///////////////////////////////////////////////////////////////// ### BLOCK ### OF LINES TOO LONG !!! ###
//,------------------------------------------------------.  ,------------------------------------------------------------------.
      XXXXXXX,   XXXXXXX,   XXXXXXX,   XXXXXXX, XXXXXXX,        C(G(KC_UP)), TD(PVIM_uU), A(KC_LEFT), A(KC_RGHT), TD(PVIM_pP),
//|----------+----------|----------+----------+----------|  |--------------+------------+-----------+-----------+--------------|
      _______,   _______,   _______,   _______, XXXXXXX,             PVIM_H,     KC_LEFT,      KC_UP,    KC_DOWN,     KC_RGHT,
//|----------+----------|----------+----------+----------|  |--------------+------------+-----------+-----------+--------------|
    MO(_ZVIM), MO(_XVIM), MO(_CVIM), MO(_FVIM), XXXXXXX,      C(G(KC_DOWN)),  A(KC_HOME), A(KC_PGUP), A(KC_PGDN),   A(KC_END),    
//'------------------------------------------------------'  '------------------------------------------------------------------'

//    MO(_ZVIM), MO(_XVIM), MO(_CVIM), MO(_FVIM), XXXXXXX,      C(G(KC_DOWN)),    PVIM_M,    PVIM_ES,    PVIM_BS,     PVIM_EN,
//                                    ,--------+---------,  ,--------+----------,
                                        _______, _______,     _______,   KC_UP,
//                                    |--------+---------|  |--------+----------|
                                                 _______,     _______,
...
/* Keymap _DVIM 04: _DVIM layer  // delete layer
 * ,----------------------------------.  ,----------------------------------.
 ...
 */
// _DVIM layer 04
[_DVIM] = KEYMAP(  // layer 04 : _DVIM layer  // delete layer
//,---------------------------------------------.,------------------------------------------------------.
...
/* Keymap _CVIM 05: _CVIM layer  // power edition layer
 * ,----------------------------------.  ,----------------------------------.
...
*/
// _CVIM layer 05
[_CVIM] = KEYMAP(  // layer 05 : _CVIM layer  // power edition VIM layer
//////////////////////////////////////////////////////////////////////////////////////////////////////###
/*
//,----------------------------------------------.        ,---------------------------------------------.
...
/* Keymap _XVIM 06: _XVIM layer  // movement VIM layer
 * ,----------------------------------.  ,----------------------------------.
 * |      |      |      |      |      |  |   5  |  15  |  25  |  50  |  75  |
 * |      |      |      |      |      |  |XXXXXX| lines|XXXXXX| lines|XXXXXX|
 * |      |      |      |      |      |  |XXXXXX|  Up  |XXXXXX|  Up  |XXXXXX|
 * |------+------+------+------+------|  |------+------+------+------+------|
 ...
 * |      |      |      |@@@@@@|      |  |   5  |  15  |  25  |  50  |  75  |
 * |      |@@@@@@|      |@@@@@@|      |  |XXXXXX| lines|XXXXXX| lines|XXXXXX|
 * |      |@@@@@@|      |@@@@@@|      |  |XXXXXX| Down |XXXXXX| Down |XXXXXX|
 * '----------------------------------'  '----------------------------------'
*/
// _XVIM layer 06
[_XVIM] = KEYMAP(  // layer 06 : _XVIM layer  // movement VIM layer
//////////////////////////////////////////////////////////////////////////////////////////////////////###
//,----------------------------------------------.         ,--------------------------------------------.
    XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,            XXXXXXX, XVIM_U, XXXXXXX,  XVIM_O, XXXXXXX,
//|--------+--------+--------+--------+----------|         |-------+-------+--------+--------+----------|
    _______, _______, _______, _______, XXXXXXX,             XVIM_H, XVIM_J,  XVIM_K, XXXXXXX, XXXXXXX,
//|--------+--------+--------+--------+----------|         |-------+-------+--------+--------+----------|
    _______, _______, _______, _______, XXXXXXX,            XXXXXXX, XVIM_M, XXXXXXX, XVIM_BS, XXXXXXX,
//'----------------------------------------------'         '--------------------------------------------'
//                            ,--------+---------,         ,--------+----------,
                                _______, _______,            _______, _______,
//                            |--------+---------|         |--------+----------|
...
/* Keymap _ZVIM 07: _ZVIM layer  // alignment VIM layer
 * ,----------------------------------.  ,----------------------------------.
 * |      |      |      |      |      |  XXSuper|      | Move | Move XXPasteX
 * |      |      |      |      |      |  |script|      |3Words|3Words|asRich|
 * |      |      |      |      |      |  |      |      | Left | Right| Text |
 * |------+------+------+------+------|  |------+------+------+------+------|
 * |      |      |      |      |      |  XX Un  | Align| Align| Align| Align|
 * |      |      |      |      |      |  |script|to the|in the|Justi-|to the|
 * |      |      |      |      |      |  |      | Left |Center| fied | Right|
 * |------+------+------+------+------|  |------+------+------+------+------|
 * |      |      |      |@@@@@@|      |  XXSubs |Scroll|Scroll|Scroll|Scroll|
 * |@@@@@@|      |      |@@@@@@|      |  |script| until| Page | Page | until|
 * |@@@@@@|      |      |@@@@@@|      |  |      | HOME |  UP  | DOWN |  END |
 * '----------------------------------'  '----------------------------------'
*/
// _ZVIM layer 07
[_ZVIM] = KEYMAP(  // layer 07 : _ZVIM layer  // alignment VIM layer
//////////////////////////////////////////////////////////////////////////////////////////////////////###
//,----------------------------------------------.        ,---------------------------------------------.
    XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX,           XXXXXXX, XXXXXXX,  ZVIM_I,  ZVIM_O, XXXXXXX,
//|--------+--------+--------+--------+----------|        |-------+--------+--------+--------+----------|
    _______, _______, _______, _______, XXXXXXX,           XXXXXXX,  ZVIM_J,  ZVIM_K,  ZVIM_L, ZVIM_SP,
//|--------+--------+--------+--------+----------|        |-------+--------+--------+--------+----------|
    _______, _______, _______, _______, XXXXXXX,           XXXXXXX,  ZVIM_M, ZVIM_ES, ZVIM_BS, ZVIM_EN,
//'----------------------------------------------'        '---------------------------------------------'
//////////////////////////////////////////////////////////////////////////////////////////////////////###
//                           ,--------+---------,         ,--------+----------,
                               _______, _______,            _______, _______,
...
*/
// _MOUS layer 08
[_MOUS] = KEYMAP(  // layer 08: mouse layer
//////////////////////////////////////////////////////////////////////////////////////////////////////###
//,----------------------------------------------.       ,----------------------------------------------.
 ...
 * '-----------------------------------'  '-----------------------------------'
*/
// _ACCN accent layer 09
[_ACCN] = KEYMAP(  // layer 09 : _ACCN layer 
//,------------------------------------------------. ,--------------------------------------------------.
...
/* Keymap _FUNC 11: functions Layer
 * ,------------------------------------.  ,----------------------------------.
 * |      |      |        |      |      |  |      |      |      |      |      |
...
*/
// _FUNC layer 11
[_FUNC] = KEYMAP(  // layer 11 : multiple apps layer
// LSFT_T(KC_A) = MT(MOD_LSFT, KC_A)
//,------------------------------------------.           ,----------------------------------------------.
...
/* Keymap _DALY 12: DAiLY commands layer
 * ,----------------------------------.  ,----------------------------------.
 * |@@@@@@|      |Captur|Captur|      |  |      |      |      |      |@@@@@@|
...
*/
// DALY layer 12
[_DALY] = KEYMAP(  // layer 12: DAiLY commands layer
//,--------------------------------------------------.,-------------------------------------------------.
...
///////////////////////////////////////////////////////////////// ### BLOCK ### OF LINES TOO LONG !!! ###
//,-------------------------------------------------------------------------.  ,---------------------------------------------------------------------------.
MY_STEP_INDICAT,    TD(LOGLCK),      MO(_RGBL),      MY_RESET,     XXXXXXX,            VOL_8,       VOL_1, LT(_RGBL, KC__VOLUP), KC__VOLDOWN,    KC__MUTE,
//|------------+--------------+---------------+--------------+--------------|  |------------+------------+---------------------+------------+--------------|
...
*/
// _APPS layer 14
[_APPS] = KEYMAP(  // layer 14 : apps layer
// ###
///////////////////////////////////////////////////////////////// ### BLOCK ### OF LINES TOO LONG !!! ###
//,-----------------------------------------------------------------.  ,------------------------------------------------------------------.
...
*/
      ...
      case MY_RESET:  reset_my_keyboard_function();
                      return false;
                      
      case THUMB_L4_POWR_DALY_RESET:
      ...
      case THUMB_L2_FVIM_FVIM_MOUS_DVIM_FUNC:
...
*/      
      case APP_T_TERMI: callApp("t");                         return false; break; // T erminal
      case APP_Y_TYPIN: callApp("y");                         return false; break; // t Y pinator

      case APP_U_SUBLI: callApp("u");                         return false; break; // s U blime Text
      case APP_I_TEDIT: callApp("i");                         return false; break; // textEd I t

      case APP_O_OMNIF: callApp("o");                         return false; break; // O mnifocus
      case APP_P_SPREF: callApp("p");                         return false; break; // system P references
/*
...
// _FVIM
   ...
   // ... except for the 'H' key:
      case PVIM_H: pvim("h");  return false;
/*
      case PVIM_M: pvim("m");  return false;
      case PVIM_ES:pvim("\e"); return false;
      case PVIM_BS:pvim("\b"); return false;
      case PVIM_EN:pvim("\n"); return false;
   // _fvim
*/
// [INFO]
...
//    case CVIM_H: cvim("h");  return false;
...
// _ZVIM
//    case ZVIM_Y: zvim("y");  return false;
//    case ZVIM_U: zvim("u");  return false;
      case ZVIM_I: zvim("i");  return false;
      case ZVIM_O: zvim("o");  return false;
//    case ZVIM_P: zvim("p");  return false;

//    case ZVIM_H: zvim("h");  return false;
      case ZVIM_J: zvim("j");  return false;
      case ZVIM_K: zvim("k");  return false;
      case ZVIM_L: zvim("l");  return false;
      case ZVIM_SP:zvim(" ");  return false;

//    case ZVIM_N: zvim("n");  return false;
      case ZVIM_M: zvim("m");  return false;
      case ZVIM_ES:zvim("\e"); return false;
      case ZVIM_BS:zvim("\b"); return false;
      case ZVIM_EN:zvim("\n"); return false;
...
*/
      ...
      case CH_RED:  rgblight_sethsv_noeeprom(HSV_RED);
                    set_default_hsv();
                    return false;
      case CH_CORL: rgblight_sethsv_noeeprom(HSV_CORAL);
                    set_default_hsv();
                    return false;

      case CH_ORNG: rgblight_sethsv_noeeprom(HSV_ORANGE);
                    set_default_hsv();
                    return false;

      case CH_GOLR: rgblight_sethsv_noeeprom(HSV_GOLDENROD);
                    set_default_hsv();
                    return false;
      case CH_GOLD: rgblight_sethsv_noeeprom(HSV_GOLD);
                    set_default_hsv();
                    return false;

//Save the original setting to EEPROM before layer change, and read it back and set it again after returning to base layer.

      case CH_YLLW: rgblight_sethsv(HSV_YELLOW);   //////////////////////////////////////////////////////
                    set_default_hsv();
                    return false;

      case SAV_COL: flashing_LEDs(10, RGB_AZURE, RGB_PURPLE);
...
ROW 2
*/
      case CH_CHRT: rgblight_sethsv_noeeprom(HSV_CHARTREUSE);
                    set_default_hsv();
                    return false;

      case CH_GREN: rgblight_sethsv_noeeprom(HSV_GREEN);
                    set_default_hsv();   
                    return false;

      case CH_SPRG: rgblight_sethsv_noeeprom(HSV_SPRINGGREEN);
                    set_default_hsv();   
                    return false;

      case CH_TRQS: rgblight_sethsv_noeeprom(HSV_TURQUOISE);
                    set_default_hsv();   
                    return false;

      case CH_TEAL: rgblight_sethsv_noeeprom(HSV_TEAL);
                    set_default_hsv();   
                    return false;

      case CH_WHIT: rgblight_sethsv_noeeprom(HSV_WHITE);
                    set_default_hsv();   
                    return false;
...
ROW 3
*/
      case CH_CYAN: rgblight_sethsv_noeeprom(HSV_CYAN);
                    set_default_hsv();   
                    return false;

      case CH_AZUR: rgblight_sethsv_noeeprom(HSV_AZURE);
                    set_default_hsv();   
                    return false;

     case CH_BLUE: rgblight_sethsv_noeeprom(HSV_BLUE);
                    set_default_hsv();
                    return false;

      case CH_PRPL: rgblight_sethsv_noeeprom(HSV_PURPLE);
                    set_default_hsv();   
                    return false;

      case CH_MGNT: rgblight_sethsv(HSV_MAGENTA);  //////////////////////////////////////////////////////
                    set_default_hsv();
                    return false;

      case CH_PINK: rgblight_sethsv_noeeprom(HSV_PINK);
                    set_default_hsv();
                    return false;

      case CH_EMPT: rgblight_sethsv_noeeprom(HSV_EMPTY);
                    set_default_hsv();
                    return false;
...
/*
      ...
      case ZVIM_BS:
      case ZVIM_EN:
                    return false; // Skip all further processing of ALL these keys when released
*/
...
// [_RGBL]
      ...
      case RGB_SAD:
      case RGB_VAI:
      case RGB_VAD: set_default_hsv(); return false;
