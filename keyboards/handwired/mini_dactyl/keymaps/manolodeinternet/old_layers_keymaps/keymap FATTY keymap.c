...
//   APPS_MODE
     BCK_DV_AP    // Backspace when tapping, _NUMB when holding 
    ,DEL_SY
    ,ESC_DA_AP
...
//////////////////////////////////////////////////////////////////////////////////////////
//                                                                                      //
// [TAPDANCE] [_DFLT] L_THUMB_1 (BCK_DV_AP)                                             //
//                                                                                      //
...
//instantalize an instance of 'tap' for the 'BCK_DV_AP' tap dance.
static tap BCK_DV_AP_tap_state = {
  .is_press_action = true,
  .state = 0
};

void BCK_DV_AP_f_always(qk_tap_dance_state_t *state, void *user_data) {
  rgblight_sethsv_noeeprom(HSV_RED);
}

void BCK_DV_AP_finished (qk_tap_dance_state_t *state, void *user_data) {
  BCK_DV_AP_tap_state.state = cur_dance(state);
  switch (BCK_DV_AP_tap_state.state) {

    case   SINGLE_HOLD: layer_on(_DVIM);          break;
    ...
    case    SINGLE_TAP:
    case GUI_plus_HOLD: register_code(KC_BSPC);   break;
  }
}

void BCK_DV_AP_reset (qk_tap_dance_state_t *state, void *user_data) {
  switch (BCK_DV_AP_tap_state.state) {

    case   SINGLE_HOLD: layer_off(_DVIM);         break;

    case ALT_plus_HOLD: layer_off(_APPS);         break;

    case    SINGLE_TAP:    
    case GUI_plus_HOLD: unregister_code(KC_BSPC);
                        show_RGB_LEDs();          break;
  }

  BCK_DV_AP_tap_state.state = 0;
}
//                                                                                      //
// [tapdance] [_dflt] l_thumb_1 (bck_dv_ap)                                             //
//                                                                                      //
...
//////////////////////////////////////////////////////////////////////////////////////////
//                                                                                      //
// [TAPDANCE] [_DFLT] L_THUMB_3 (DEL_SY)                                                //
//                                                                                      //
...
void DEL_SY_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (DEL_SY_tap_state.state) {
    ...
    case    SINGLE_TAP:    
    case GUI_plus_HOLD: register_code(KC_DEL);   break;
...
void DEL_SY_reset (qk_tap_dance_state_t *state, void *user_data) {
  switch (DEL_SY_tap_state.state) {
    ...
    case    SINGLE_TAP:
    case GUI_plus_HOLD: unregister_code(KC_DEL); break;

    case   SINGLE_HOLD: layer_off(_NUMB);        break;
  }
  show_RGB_LEDs();
...
//////////////////////////////////////////////////////////////////////////////////////////
//                                                                                      //
// [TAPDANCE] [_DFLT] R_THUMB_1 (ESC_DA_AP)                                             //
//                                                                                      //
//  E S C A P E    /    D A L Y    L A Y E R    /    A P P S    L A Y E R               //
//                                                                                      //
//  R_THUMB_1:  *   ESCAPE,                                                             //
//              @ [_DALY] LAYER                                                         //
//              ⌥ [_APPS] LAYER                                                         //
...
//////////////////////////////////////////////////////////////////////////////////////////
//
//instantalize an instance of 'tap' for the 'ESC_DA_AP' tap dance.
static tap ESC_DA_AP_tap_state = {
  .is_press_action = true,
  .state = 0
};

void ESC_DA_AP_f_always(qk_tap_dance_state_t *state, void *user_data) {
  rgblight_sethsv_noeeprom(HSV_RED);
}

void ESC_DA_AP_finished (qk_tap_dance_state_t *state, void *user_data) {
  ESC_DA_AP_tap_state.state = cur_dance(state);
  switch (ESC_DA_AP_tap_state.state) {

    case    SINGLE_TAP: register_code(KC_ESC);    break;

    case   SINGLE_HOLD: layer_on(_DALY);          break;
...
void ESC_DA_AP_reset (qk_tap_dance_state_t *state, void *user_data) {
  switch (ESC_DA_AP_tap_state.state) {

    case    SINGLE_TAP: unregister_code(KC_ESC);  break;

    case   SINGLE_HOLD: layer_off(_DALY);         break;

    case ALT_plus_HOLD: layer_off(_APPS);         break;
  }

  ESC_DA_AP_tap_state.state = 0;
}
//                                                                                      //
// [tapdance] [_dflt] r_thumb_1 (esc_da_ap)                                             //
//                                                                                      //
...
void APPS_MODE_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (APPS_MODE_tap_state.state) {
    ...
    case GUI_plus_HOLD:        multi_apps = true;
...
[BCK_DV_AP]=ACTION_TAP_DANCE_FN_ADVANCED_TIME(BCK_DV_AP_f_always,BCK_DV_AP_finished,BCK_DV_AP_reset, 100)
 ,[DEL_SY] =ACTION_TAP_DANCE_FN_ADVANCED_TIME(DEL_SY_f_always,      DEL_SY_finished,   DEL_SY_reset, 100)
 ,[ESC_DA_AP] = ACTION_TAP_DANCE_FN_ADVANCED (NULL,              ESC_DA_AP_finished,ESC_DA_AP_reset)
...
//                                              TD(APPS_MODE),    THUMB_L4_DALY_RGBL_POWR,    THUMB_R4_POWR_RGBL_DALY,       MO(_ACCN),
                                            LT(_MOUS, KC_TAB),                  MO(_FUNC),    THUMB_R4_POWR_RGBL_DALY,       OSL(_ACCN),
//                        |----------------------------------+---------------------------|   |------------------------+--------------------|
                                                                                  XXXXXXX,                     XXXXXXX,
//                                                           |---------------------------|   |------------------------|
//               MO(_NUMB), THUMB_L2_FVIM_CAPSL_MOUS_DVIM_FUNC,                MO(_SYMB),             LT(_SYMB, KC_ENT), THUMB_R2_SP_FVIM_MOUS_DVIM_FUNC, LT(_NUMB, KC_ESC)
                TD(BCK_DV_AP), THUMB_L2_FVIM_CAPSL_MOUS_DVIM_FUNC,               TD(DEL_SY),             LT(_SYMB, KC_ENT), THUMB_R2_SP_FVIM_MOUS_DVIM_FUNC, TD(ESC_DA_AP)
//       LT(_NUMB, KC_BSPC), THUMB_L2_FVIM_CAPSL_MOUS_DVIM_FUNC,               TD(DEL_SY),             LT(_SYMB, KC_ENT), THUMB_R2_SP_FVIM_MOUS_DVIM_FUNC, LT(_NUMB, KC_ESC)
